...
type mockAutoConfigDelegate struct {
	mock.Mock
}

func (m *mockAutoConfigDelegate) GetConfig() *Config {
	ret := m.Called()
...
func (m *mockAutoConfigDelegate) CreateACLToken(template *structs.ACLToken) (*structs.ACLToken, error) {
...
func (m *mockAutoConfigDelegate) DatacenterJoinAddresses(segment string) ([]string, error) {
...
func (m *mockAutoConfigDelegate) TLSConfigurator() *tlsutil.Configurator {
	ret := m.Called()
	// this handles converting an untyped nil to a typed nil
...
func (m *mockAutoConfigDelegate) ForwardRPC(method string, info structs.RPCInfo, args, reply interface{}) (bool, error) {
...
func TestAutoConfig_baseConfig(t *testing.T) {
	...
	for name, tcase := range cases {
		t.Run(name, func(t *testing.T) {
			delegate := &mockAutoConfigDelegate{}
			delegate.On("GetConfig").Return(&tcase.serverConfig).Once()
...
func TestAutoConfig_updateTLSSettingsInConfig(t *testing.T) {
	...
	for name, tcase := range cases {
		t.Run(name, func(t *testing.T) {
			...
			delegate := &mockAutoConfigDelegate{}
			delegate.On("TLSConfigurator").Return(configurator).Once()
...
func TestAutoConfig_updateGossipEncryptionInConfig(t *testing.T) {
	...
	for name, tcase := range cases {
		t.Run(name, func(t *testing.T) {
			...
			delegate := &mockAutoConfigDelegate{}
			delegate.On("GetConfig").Return(cfg).Once()
...
func TestAutoConfig_updateACLsInConfig(t *testing.T) {
	...
	for name, tcase := range cases {
		t.Run(name, func(t *testing.T) {
			delegate := &mockAutoConfigDelegate{}
			delegate.On("GetConfig").Return(&tcase.config).Once()
			...
			if tcase.expectACLToken {
				delegate.On("CreateACLToken", expectedTemplate).Return(testToken, tcase.err).Once()
			}

			ac := AutoConfig{delegate: delegate}
...
func TestAutoConfig_updateJoinAddressesInConfig(t *testing.T) {
	...
	delegate := &mockAutoConfigDelegate{}
	delegate.On("DatacenterJoinAddresses", "").Return(addrs, nil).Once()
